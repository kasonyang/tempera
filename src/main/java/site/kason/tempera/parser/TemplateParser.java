...
import site.kason.tempera.lexer.*;
...
public class TemplateParser {
    ...
    private TemplateAstLoader templateAstLoader;
    ...
    public TemplateParser(String className, String templateName, TokenStream ts, TemplateAstLoader templateAstLoader, TemplateClassLoader classParser) {
        ...
        this.templateAstLoader = templateAstLoader;
        this.astLoader = new JavaAstLoader(AstLoader.BASE_AST_LOADER, TemplateParser.class.getClassLoader());
        this.tokenStream = new BufferedTokenStream(ts);
        ClassNode cn = new ClassNode();
        cn.name = className;
        cn.modifier = Modifier.PUBLIC;
        try {
            cn.setSuperType(Types.getClassType(astLoader.loadAst(Renderer.class.getName())));
        ...
        typeNameResolver.setAstLoader(astLoader);
    ...
    private Type scanType() throws SemanticException, LexException {
        ...
        if (isToken(LT)) {
            ...
            expect(GT);
            try {
                type = Types.getClassType(this.templateAstLoader.loadAst(fullName), pms.toArray(new Type[pms.size()]));
            } catch (AstNotFoundException ex) {
                throw Exceptions.classNotFound(typeToken, fullName);
            }
        } else {
            try {
                type = Types.getClassType(this.templateAstLoader.loadAst(fullName));
            } catch (AstNotFoundException ex) {
                throw Exceptions.classNotFound(typeToken, fullName);
            }
        }
    ...
    private Statement layout() throws LexException, TemplateNotFoundException, IOException {
        ...
        expect(END_TAG);
        ClassNode parentAst = this.templateAstLoader.loadTemplateAst(parentId.getText());
